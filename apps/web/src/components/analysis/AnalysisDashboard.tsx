...
export function AnalysisDashboard({ jobId }: AnalysisDashboardProps) {
  ...
  const getReportMarkdown = () => {
    ...
    md += `---\n\n*Generated by Luminae — Built by DevRay Labs*\n`
    return md
  }
  ...
  const exportDoc = () => {
    ...
    const documentTemplate = `
      ...
      <head>
        ...
        <style>
          ...
          h1 { font-size: 24pt; color: #1e3a8a; margin-bottom: 12pt; border-bottom: 2px solid #e2e8f0; padding-bottom: 6pt; }
          h2 { font-size: 18pt; color: #1e40af; margin-top: 18pt; margin-bottom: 10pt; }
          h3 { font-size: 14pt; color: #1e3a8a; margin-top: 14pt; margin-bottom: 8pt; }
          ...
          blockquote { border-left: 4px solid #94a3b8; padding-left: 12px; margin-left: 0; color: #475569; font-style: italic; }
          pre, code { font-family: Consolas, monospace; background-color: #f8fafc; font-size: 10pt; }
        </style>
      </head>
      <body>
        \${html}
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `luminae-report-\${title.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 40)}.doc`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
    URL.revokeObjectURL(url)
      ...
      <div className={`bg-card rounded-xl shadow-sm border overflow-hidden flex flex-col ${hasPdf ? 'h-full' : ''}`}>
        ...
        <div className="flex items-center justify-end px-6 pt-4 gap-2 border-b pb-4 bg-muted/30">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-indigo-700 bg-indigo-50 border border-indigo-200 rounded hover:bg-indigo-100 transition-colors"
          >
